...
public class Extra3Learner extends Learner {
  private LibSVM model;
  private BM25Scorer bm25scorer;
  private SmallestWindowScorer windowScorer;
  private Embedding wordVectors;
  
  public Extra3Learner(Map<String, Double> idf){
    try{
      model = new LibSVM();
    } catch (Exception e){
      e.printStackTrace();
    ...
    wordVectors = new Embedding("wordVectors.txt", false);
  }
  ...
  public Instances extract_train_features(String train_data_file,
    ...
    double[][] vec = new double[wordVectors.get("$1").numRows()][1];
...
//        values[8] = d.url.contains(".pdf") ? 1 : 0;
        ...
        SimpleMatrix c = new SimpleMatrix(vec);
        for (String w : q.words) {
          c = c.plus(wordVectors.get(w));
        }
        String[] titletokens = d.title.split(" ");
        SimpleMatrix tmp = new SimpleMatrix(vec);
        if (d.title != null && d.title.compareTo("") != 0) {
          for (String w : titletokens) {
            tmp = tmp.plus(wordVectors.get(w));
          }
        }
//        values[10] = c.transpose().mult(tmp).get(0);
...
//          values[8] = d.url.contains(".pdf") ? 1 : 0;
          ...
          String[] titletokens = d.title.split(" ");
          SimpleMatrix tmp = new SimpleMatrix(vec);
//          System.out.println("url = " + d.url + " title = " + d.title);
          if (d.title != null && d.title.compareTo("") != 0) {
          for (String w : titletokens) {
//            System.out.println("word = " + w);
//            System.out.println(wordVectors.get(w));
            tmp = tmp.plus(wordVectors.get(w));
          }
          }
//          values[10] = c.transpose().mult(tmp).get(0);
